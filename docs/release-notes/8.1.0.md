...
### Sentinel listener shutdown ([#3839](https://github.com/redis/jedis/issues/3839))

Sentinel listeners now preserve shutdown requests made before their threads start.
If shutdown occurs while a listener is creating a connection, the new connection
is closed without entering the subscription loop. This applies to
`SentineledConnectionProvider` and the legacy `JedisSentinelPool`.
