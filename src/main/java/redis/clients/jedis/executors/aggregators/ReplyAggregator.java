...
/**
 * Stateful aggregator that keeps aggregating replies of the same type
 * according to the given ResponsePolicy.
 *
 * The proper underlying aggregator is lazily created on the first non-null value.
...
class ReplyAggregator<T> implements Aggregator<T, T> {
    ...
    /**
     * Returns the aggregated result so far.
     * Returns null if no meaningful value has been added.
     */
    @Override
    public T getResult() {
