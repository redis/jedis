...
public interface ClusterCommands {
  ...
  /**
   * Set a specific config epoch in a fresh node. It only works when the nodes' table of the node is
   * empty or when the node current config epoch is zero.
   * @param configEpoch
   * @return OK
   */
   ...
   * @return BUMPED if the epoch was incremented, or STILL if the node already has the greatest
   *         config epoch in the cluster.
   */
  ...
  /**
   * {@code CLUSTER SHARDS} returns details about the shards of the cluster. This command replaces
   * the {@code CLUSTER SLOTS} command from Redis 7, by providing a more efficient and extensible
  ...
  /**
   * return the information of all such peer links as an array, where each array element is a map
   * that contains attributes and their values for an individual link.
