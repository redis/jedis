...
public abstract class JedisClusterConnectionHandler {
	...
	public JedisClusterConnectionHandler(Set<HostAndPort> nodes, final GenericObjectPoolConfig poolConfig, int timeout, String password) {
		//add password if needed
		this.cache = new JedisClusterInfoCache(poolConfig, timeout,password);
		initializeSlotsCache(nodes,password, poolConfig);
	}
	...
	private void initializeSlotsCache(Set<HostAndPort> startNodes, String password, GenericObjectPoolConfig poolConfig) {
		for (HostAndPort hostAndPort : startNodes) {
			Jedis jedis = new Jedis(hostAndPort.getHost(), hostAndPort.getPort());
			try {
				// need auth if cluster running in requirepass mode
				if (password != null)
					jedis.auth(password);
