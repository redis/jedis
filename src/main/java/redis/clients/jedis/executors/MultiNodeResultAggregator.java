 ...
 * <p>
 * This class centralizes the logic for collecting results and errors from multi-node
 * command execution (broadcast commands, multi-shard commands) and determining the
 * final result based on the {@link CommandFlagsRegistry.ResponsePolicy}.
 * <p>
...
final class MultiNodeResultAggregator<T> {
   ...
   * <p>
   * Use this method when the node information is not readily available,
   * such as in multi-shard commands where extracting the node would require
   ...
   * <p>
   * For {@code ONE_SUCCEEDED} policy: returns the aggregated result if at least one node
   * succeeded, throws {@link JedisBroadcastException} only if all nodes failed.
   * <p>
   * For all other policies: throws {@link JedisBroadcastException} if any node failed,
   * otherwise returns the aggregated result.
   *
