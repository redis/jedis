...
public class AsyncListCommandsTest extends AsyncJedisCommandTestBase {
	...
	List<byte[]> brange = BYTE_ARRAY_LIST_CALLBACK
	...
	assertEquals(srcExpected,
		STRING_LIST_CALLBACK.getResponseWithWaiting(1000));

	asyncJedis.lrange(STRING_LIST_CALLBACK.withReset(), "dst", 0, 1000);
	assertEquals(dstExpected,
		STRING_LIST_CALLBACK.getResponseWithWaiting(1000));
    ...
    @Test
    public void linsert() {
	asyncJedis.linsert(LONG_CALLBACK.withReset(), "foo",
		Client.LIST_POSITION.BEFORE, "bar", "car");
	long status = LONG_CALLBACK.getResponseWithWaiting(1000);
	assertEquals(0, status);
	...
	asyncJedis.linsert(LONG_CALLBACK.withReset(), "foo",
		Client.LIST_POSITION.AFTER, "a", "b");
	status = LONG_CALLBACK.getResponseWithWaiting(1000);
	assertEquals(2, status);
	...
	// Binary
	asyncJedis.linsert(LONG_CALLBACK.withReset(), bfoo,
		Client.LIST_POSITION.BEFORE, bbar, bcar);
	long bstatus = LONG_CALLBACK.getResponseWithWaiting(1000);
	assertEquals(0, bstatus);
	...
	asyncJedis.linsert(LONG_CALLBACK.withReset(), bfoo,
		Client.LIST_POSITION.AFTER, bA, bB);
	bstatus = LONG_CALLBACK.getResponseWithWaiting(1000);
	assertEquals(2, bstatus);
