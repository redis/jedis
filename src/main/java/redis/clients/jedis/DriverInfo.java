 ...
 * <p>
 * This class is used to identify the client library and any upstream drivers (such as Spring Data Redis 
 * or Spring Session) when connecting to Redis. The information is sent via the {@code CLIENT SETINFO} command.
 * <p>
...
public final class DriverInfo implements Serializable {
   ...
   * contain only lowercase letters, digits, hyphens, and underscores. Dots are only allowed after digits
   * (for Scala cross-version naming like akka-redis_2.13). Follows Maven artifactId naming conventions 
   * but also allows underscores and version-specific dots.
   *
  ...
  /**
   * Set of brace characters that are not allowed in driver names or versions. These characters are used 
   * to delimit the driver information in the formatted output and would break parsing.
   */
  private static final Set<Character> BRACES = Collections.unmodifiableSet(
   ...
   * <p>
   * If a legacy suffix is set, returns the name followed by the suffix in parentheses.
   * Otherwise, if upstream drivers are present, returns the name followed by upstream drivers in
   * parentheses, separated by semicolons.
   * If neither is set, returns just the name.
   * <p>
   * Examples:
   * <ul>
   ...
   * <li>{@code "jedis(spring-session_v3.3.0;spring-data-redis_v3.2.0)"} - multiple upstream drivers</li>
   * </ul>
   *
  ...
  public static class Builder {
     ...
     * <p>
     * Upstream drivers are prepended to the list, so the most recently added driver appears first in
     * the formatted output.
     * <p>
     * The driver name must follow Maven artifactId naming conventions: lowercase letters, digits, hyphens,
     * and underscores only, starting with a lowercase letter. Dots are only allowed after digits (for
     * Scala cross-version naming like akka-redis_2.13).
     * <p>
     * Both values must not contain spaces, newlines, non-printable characters, or brace characters as
     * these would violate the format of the Redis CLIENT LIST reply.
     *
     ...
     * @return this builder
     * @throws JedisValidationException if the driver name or version is {@code null} or has invalid format
     * @see <a href="https://maven.apache.org/guides/mini/guide-naming-conventions.html">Maven Naming Conventions</a>
     * @see <a href="https://redis.io/docs/latest/commands/client-setinfo/">CLIENT SETINFO</a>
     */
   ...
   * <p>
   * Additionally validates Redis CLIENT LIST constraints: no spaces, newlines, non-printable characters, 
   * or braces.
   *
   * @param driverName the driver name to validate
   * @throws JedisValidationException if the driver name does not follow the expected naming conventions
   * @see <a href="https://maven.apache.org/guides/mini/guide-naming-conventions.html">Maven Naming Conventions</a>
   * @see <a href="https://redis.io/docs/latest/commands/client-setinfo/">CLIENT SETINFO</a>
   */
  ...
  /**
   * Validates that the driver version does not contain characters that would violate the format of the
   * Redis CLIENT LIST reply: no spaces, newlines, non-printable characters, or brace characters.
   *
