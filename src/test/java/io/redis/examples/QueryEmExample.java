...
public class QueryEmExample {
  ...
  public void run() {
    ...
    // STEP_START em3
    SchemaField[] emailSchema = { TextField.of("$.email").as("email") };

    jedis.ftCreate("idx:email", new FTCreateParams().addPrefix("key:").on(IndexDataType.JSON),
      emailSchema);

    jedis.jsonSet("key:1", Path2.ROOT_PATH, "{\"email\": \"test@redis.com\"}");

    SearchResult res4 = jedis.ftSearch("idx:email",
      RediSearchUtil.escapeQuery("@email{test@redis.com}"), new FTSearchParams().dialect(2));
