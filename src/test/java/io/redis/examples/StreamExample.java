...
public class StreamExample {
  ...
  public void run(){
    ...
    List<StreamEntry> res13 = jedis.xrange("race:france","-","+",2);
    System.out.println(res13); // >>> [1701764887638-0 {rider=Castilla, speed=30.2, location_id=1, position=1}, 1701764887638-1 {rider=Norem, speed=28.8, location_id=1, position=3}]
    //STEP_END

    //STEP_START xrange_step_2
    List<StreamEntry> res14 = jedis.xrange("race:france",String.valueOf(System.currentTimeMillis()-1000)+"-0","+",2);
    ...
    List<StreamEntry> res16 = jedis.xrevrange("race:france","+","-",1);
    System.out.println(res16); // >>> [1701765218592-0 {rider=Castilla, speed=29.9, location_id=1, position=2}]
    //STEP_END

    //STEP_START xread
    List<Map.Entry<String, List<StreamEntry>>> res17= jedis.xread(XReadParams.xReadParams().count(2),new HashMap<String,StreamEntryID>(){{put("race:france",new StreamEntryID());}});
    System.out.println(
      res17
    ...
    String res18 = jedis.xgroupCreate("race:france","france_riders",StreamEntryID.LAST_ENTRY,false);
    System.out.println(res18); // >>> OK
    //STEP_END

    //STEP_START xgroup_create_makesream
    String res19 = jedis.xgroupCreate("race:italy","italy_riders",StreamEntryID.LAST_ENTRY,true);
    ...
    List<Map.Entry<String, List<StreamEntry>>> res20 = jedis.xreadGroup("italy_riders","Alice", XReadGroupParams.xReadGroupParams().count(1),new HashMap<String,StreamEntryID>(){{put("race:italy",StreamEntryID.UNRECEIVED_ENTRY);}});
    System.out.println(res20); // >>> [race:italy=[1701766299006-0 {rider=Castilaa}]]
    //STEP_END

    //STEP_START xgroup_read_id
    List<Map.Entry<String, List<StreamEntry>>> res21 = jedis.xreadGroup("italy_riders","Alice", XReadGroupParams.xReadGroupParams().count(1),new HashMap<String,StreamEntryID>(){{put("race:italy",new StreamEntryID());}});
    System.out.println(res21); // >>> [race:italy=[1701766299006-0 {rider=Castilaa}]]
    //STEP_END

    //STEP_START xack
    long res22 = jedis.xack("race:italy","italy_riders",id1);
    System.out.println(res22); // >>> 1

    List<Map.Entry<String, List<StreamEntry>>> res23 = jedis.xreadGroup("italy_riders","Alice", XReadGroupParams.xReadGroupParams().count(1),new HashMap<String,StreamEntryID>(){{put("race:italy",new StreamEntryID());}});
    System.out.println(res23); // >>> [race:italy=[]]
    //STEP_END

    //STEP_START xgroup_read_bob
    List<Map.Entry<String, List<StreamEntry>>> res24 = jedis.xreadGroup("italy_riders","Bob", XReadGroupParams.xReadGroupParams().count(2),new HashMap<String,StreamEntryID>(){{put("race:italy",StreamEntryID.UNRECEIVED_ENTRY);}});
    System.out.println(res24); // >>> [race:italy=[1701767632261-1 {rider=Royce}, 1701767632262-0 {rider=Sam-Bodden}]]
    //STEP_END

    //STEP_START xpending
    StreamPendingSummary res25 = jedis.xpending("race:italy","italy_riders");
    System.out.println(res25.getConsumerMessageCount()); // >>> {Bob=2}
    //STEP_END

    //STEP_START xpending_plus_minus
    List<StreamPendingEntry> res26 = jedis.xpending("race:italy","italy_riders",XPendingParams.xPendingParams().start(StreamEntryID.MINIMUM_ID).end(StreamEntryID.MAXIMUM_ID).count(10));
    System.out.println(res26); // >>> [1701768567412-1 Bob idle:0 times:1, 1701768567412-2 Bob idle:0 times:1]
    //STEP_END

    //STEP_START xpending_range
    List<StreamEntry> res27 = jedis.xrange("race:italy",id2.toString(),id2.toString());
    System.out.println(res27); // >>> [1701768744819-1 {rider=Royce}]
    //STEP_END

    //STEP_START xclaim
    List<StreamEntry> res28 = jedis.xclaim("race:italy","italy_riders","Alice", 0L, XClaimParams.xClaimParams().time(60000),id2);
    System.out.println(res28); // >>> [1701769004195-1 {rider=Royce}]
    //STEP_END

    //STEP_START xautoclaim
    Map.Entry<StreamEntryID, List<StreamEntry>> res29 = jedis.xautoclaim("race:italy","italy_riders","Alice",1L,new StreamEntryID("0-0"),XAutoClaimParams.xAutoClaimParams().count(1));
    System.out.println(res29); // >>> [1701769266831-2=[1701769266831-1 {rider=Royce}]
    //STEP_END

    //STEP_START xautoclaim_cursor
    Map.Entry<StreamEntryID, List<StreamEntry>> res30 = jedis.xautoclaim("race:italy","italy_riders","Alice",1L,new StreamEntryID(id2.toString()),XAutoClaimParams.xAutoClaimParams().count(1));
    System.out.println(res30); // >>> [0-0=[1701769605847-2 {rider=Sam-Bodden}]
    //STEP_END

    //STEP_START xinfo
    StreamInfo res31 = jedis.xinfoStream("race:italy");
    System.out.println(
      res31.getStreamInfo()
    ...
    //STEP_START xinfo_group
    List<StreamGroupInfo> res32 = jedis.xinfoGroups("race:italy");
    for (StreamGroupInfo a : res32){
      System.out.println(
        a.getGroupInfo()
    ...
    List<StreamConsumersInfo> res33 = jedis.xinfoConsumers("race:italy","italy_riders");
    for (StreamConsumerInfo a : res33){
      System.out.println(
        a.getConsumerInfo()
    ...
    long res34 = jedis.xlen("race:italy");
    System.out.println(res34); // >>> 8

    List<StreamEntry> res35 = jedis.xrange("race:italy","-","+");
    System.out.println(res35); // >>> [1701771219852-0 {rider=Castilaa}, 1701771219852-1 {rider=Royce}, 1701771219853-0 {rider=Sam-Bodden}, 1701771219853-1 {rider=Prickett}, 1701771219853-2 {rider=Norem}, 1701771219858-0 {rider=Jones}, 1701771219858-1 {rider=Wood}, 1701771219859-0 {rider=Henshaw}]
    ...
    System.out.println(res36); // >>> [1701771067332-1 {rider=Henshaw}, 1701771067332-2 {rider=Smith}]
    //STEP_END

    //STEP_START xtrim
    long res37 = jedis.xtrim("race:italy",XTrimParams.xTrimParams().maxLen(10).exactTrimming());
    System.out.println(res37); /// >>> 0
    //STEP_END

    //STEP_START xtrim2
    long res38 = jedis.xtrim("race:italy",XTrimParams.xTrimParams().maxLen(10));
    System.out.println(res38); /// >>> 0
    //STEP_END

    //STEP_START xdel
    List<StreamEntry> res39 = jedis.xrange("race:italy","-","+");
    System.out.println(res39); // >>> [1701771356428-2 {rider=Henshaw}, 1701771356429-0 {rider=Smith}]

    long res40 = jedis.xdel("race:italy",id6);
    System.out.println(res40); // >>> 1
