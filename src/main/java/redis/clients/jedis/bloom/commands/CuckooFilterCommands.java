 ...
 * @see <a href= "https://oss.redislabs.com/redisbloom/Cuckoo_Commands/">RedisBloom Cuckoo Filter
 *      Documentation</a>
 */
public interface CuckooFilterCommands {
   ...
   * @return true if the item was successfully inserted, false if an error occurred
   */
   ...
   * @param key The name of the filter
   * @param insertParams An instance of CFInsertParams containing the options (CAPACITY/NOCREATE)
   * @param items One or more items to add
   * @return true if the item was added to the filter, false if the item already exists.
   */
   ...
   * @return false if the item certainly does not exist, true if the item may exist.
   */
   ...
   * @return a list of booleans where false if the item certainly does not exist, true if the item
   *         may exist.
   */
  List<Boolean> cfMExists(String key, String... items);

  /**
   * CF.DEL Deletes an item once from the filter. If the item exists only once, it will be removed
  ...
  /**
   * CF.SCANDUMP Begins an incremental save of the cuckoo filter. This is useful for large cuckoo
   * filters which cannot fit into the normal SAVE and RESTORE model. The Iterator is passed as
   * input to the next invocation of SCANDUMP . If Iterator is 0, the iteration has completed.
   * @param key Name of the filter
   * @param iterator This is either 0, or the iterator from a previous invocation of this command
   ...
   * @param key Name of the filter to restore
   * @return A Map containing Size, Number of buckets, Number of filter, Number of items inserted,
   *         Number of items deleted, Bucket size, Expansion rate, Max iteration
   */
