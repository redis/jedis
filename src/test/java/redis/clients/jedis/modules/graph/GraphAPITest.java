...
public class GraphAPITest extends RedisModuleCommandsTestBase {
  ...
  public void testIndex() {
    ...
    ResultSet createNonExistingIndexResult = client.graphQuery("social",
    ...
    ResultSet createExistingIndexResult = client.graphQuery("social",
  ...
  public void testHeader() {
    ...
    ResultSet queryResult = client.graphQuery("social",
      "MATCH (a:person)-[r:knows]->(b:person) RETURN a,r, a.age");

    Header header = queryResult.getHeader();
    assertNotNull(header);
    assertEquals("HeaderImpl{"
    // + "schemaTypes=[COLUMN_SCALAR, COLUMN_SCALAR, COLUMN_SCALAR], "
        + "schemaTypes=[SCALAR, SCALAR, SCALAR], " + "schemaNames=[a, r, a.age]}",
      header.toString());
  ...
  public void testRecord() {
    ...
    assertEquals("Node{labels=[person], id=0, "
    ...
    assertNotNull(client.graphQuery("social",
      "CREATE (:person{name:$name,age:$age, doubleValue:$doubleValue, boolValue:$boolValue})",
      params));
    assertNotNull(client.graphQuery("social", "CREATE (:person{name:'amit',age:30})"));
    assertNotNull(client.graphQuery("social",
    ...
    assertEquals(Arrays.asList(expectedNode, expectedEdge, name, (long) age, doubleValue, true,
  ...
  public void testAdditionToProcedures() {
    ...
    resultSet = client
        .graphQuery("social", "MATCH (a:worker)-[r:worksWith]->(b:worker) RETURN a,r");
    assertNotNull(resultSet.getHeader());
  ...
  public void testEscapedQuery() {
    ...
    params2.put("s1", "S\"'");
    params2.put("s2", "S'\"");
    assertNotNull(client.graphQuery("social", "MATCH (n) where n.s1=$s1 and n.s2=$s2 RETURN n",
      params2));
  ...
  public void testPath() {
    ...
    Path path01 = new PathBuilder(2).append(nodes.get(0)).append(edges.get(0)).append(nodes.get(1))
        .build();
    Path path12 = new PathBuilder(2).append(nodes.get(1)).append(edges.get(1)).append(nodes.get(2))
        .build();
  ...
  public void testNullGraphEntities() {
    ...
    resultSet = client.graphQuery("social",
  ...
  public void testCachedExecution() {
    ...
    params.put("val", 1L);
    ResultSet resultSet = client
        .graphQuery("social", "MATCH (n:N {val:$val}) RETURN n.val", params);
    assertEquals(1, resultSet.size());
    Record r = resultSet.iterator().next();
  ...
  public void testMapDataType() {
    ...
    f.put("y", (long) 2);
    expected.put("f", f);
    ResultSet res = client.graphQuery("social",
      "RETURN {a:1, b:'str', c:NULL, d:[1,2,3], e:True, f:{x:1, y:2}}");
  ...
  public void timeoutArgument() {
    ResultSet rs = client.graphQuery("social",
      "UNWIND range(0,100) AS x WITH x AS x WHERE x = 100 RETURN x", 1L);
  ...
  public void testCachedExecutionReadOnly() {
    ...
    Map<String, Object> params = new HashMap<>();
    params.put("val", 1L);
    ResultSet resultSet = client.graphReadonlyQuery("social",
      "MATCH (n:N {val:$val}) RETURN n.val", params);
    assertEquals(1, resultSet.size());
    Record r = resultSet.iterator().next();
    assertEquals(params.get("val"), r.getValue(0));
    assertFalse(resultSet.getStatistics().cachedExecution());
    ...
    for (int i = 0; i < 64; i++) {
      resultSet = client
          .graphReadonlyQuery("social", "MATCH (n:N {val:$val}) RETURN n.val", params);
    }
    assertEquals(1, resultSet.size());
    r = resultSet.iterator().next();
    assertEquals(params.get("val"), r.getValue(0));
  ...
  public void testSimpleReadOnly() {
    ...
    ResultSet rsRo = client.graphReadonlyQuery("social",
      "MATCH (a:person) WHERE (a.name = 'filipe') RETURN a.age");
