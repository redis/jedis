...
public class AggregationWithParamsTest extends RedisModuleCommandsTestBase {

  private static final String index = "aggrindex";
...
//  }
  ...
  public void testAggregationBuilderVerbatim() {
    ...
    AggregationResult res = client.ftAggregate(index, "kitti", FTAggregateParams.aggregateParams());
    assertEquals(1, res.totalResults);

    res = client.ftAggregate(index, "kitti", FTAggregateParams.aggregateParams().verbatim());
...
//  }
  ...
  public void load() {
    ...
    FTAggregateParams params = FTAggregateParams.aggregateParams()
        .load(FieldName.of("@subj1").as("a"), FieldName.of("@subj2").as("b"))
        .apply("(@a+@b)/2", "avg").sortByDesc("@avg");

    AggregationResult result = client.ftAggregate(index, params);
