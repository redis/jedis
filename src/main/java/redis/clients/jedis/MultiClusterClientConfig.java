 ...
 *         <p>
 *         Config which supports multiple cluster/database endpoint configurations that all share
 *         retry and circuit breaker configuration settings.
 *         <p>
 *         With this Config users can seamlessly failover to Disaster Recovery (DR), Backup, and
 *         Active-Active cluster(s) by using simple configuration which is passed through from
 *         Resilience4j - https://resilience4j.readme.io/docs
 *         <p>
 *         Configuration options related to automatic failback (e.g. HALF_OPENED state) are not
 *         supported and therefore not passed through to Jedis users.
 *         <p>
 */
// TODO: move
@Experimental
public final class MultiClusterClientConfig {
  ...
  private static final float CIRCUIT_BREAKER_FAILURE_RATE_THRESHOLD_DEFAULT = 50.0f; // measured as
                                                                                     // percentage
  ...
  private static final int CIRCUIT_BREAKER_SLOW_CALL_DURATION_THRESHOLD_DEFAULT = 60000; // measured
                                                                                         // in
                                                                                         // milliseconds
  private static final float CIRCUIT_BREAKER_SLOW_CALL_RATE_THRESHOLD_DEFAULT = 100.0f; // measured
                                                                                        // as
                                                                                        // percentage
  ...
  private static final List<Class<? extends Throwable>> FALLBACK_EXCEPTIONS_DEFAULT = Arrays
      .asList(CallNotPermittedException.class);
  ...
  /**
   * Wait duration increases exponentially between attempts due to the multiplier. For example, if
   * we specified an initial wait time of 1s and a multiplier of 2, the retries would be done after
   * 1s, 2s, 4s, 8s, 16s, and so on
   */
  ...
  /**
   * Configures a list of Throwable classes that are ignored and thus are not retried. This
   * parameter supports subtyping.
   */
  ...
  /**
   * When the failure rate is equal or greater than the threshold the CircuitBreaker transitions to
   * open and starts short-circuiting calls
   */
  private float circuitBreakerFailureRateThreshold;

  /**
   * Minimum number of calls required (per sliding window period) before the CircuitBreaker can
   * calculate the error rate or slow call rate. For example, if the value is 10, then at least 10
   * calls must be recorded, before the failure rate can be calculated. However, if only 9 calls
   * have been recorded, the CircuitBreaker will not transition to open even if all 9 have failed
   */
  private int circuitBreakerSlidingWindowMinCalls;

  /**
   * Used to record the outcome of calls when the CircuitBreaker is closed. If the type is
   * COUNT_BASED, the last slidingWindowSize calls are recorded and aggregated. If the type is
   * TIME_BASED, the calls of the last slidingWindowSize seconds are recorded and aggregated
   */
  private SlidingWindowType circuitBreakerSlidingWindowType;

  /**
   * Size of the sliding window which is used to record the outcome of calls when the CircuitBreaker
   * is closed
   */
  ...
  /**
   * When the percentage of slow calls is equal or greater the threshold, the CircuitBreaker
   * transitions to open and starts short-circuiting calls. CircuitBreaker considers a call as slow
   * when the call duration is greater than slowCallDurationThreshold
   */
  private float circuitBreakerSlowCallRateThreshold;

  /**
   * A list of exceptions that are recorded as a failure and thus increase the failure rate. Any
   * exception matching or inheriting from one of the list counts as a failure, unless explicitly
   * ignored via ignoreExceptions. If you specify a list of exceptions, all other exceptions count
   * as a success, unless they are explicitly ignored by ignoreExceptions
   */
  private List<Class> circuitBreakerIncludedExceptionList;

  /**
   * A list of exceptions that are ignored and neither count as a failure nor success. Any exception
   * matching or inheriting from one of the list will not count as a failure nor success, even if
   * the exceptions is part of recordExceptions
   */
