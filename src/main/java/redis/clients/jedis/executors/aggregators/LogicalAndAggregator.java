...
/**
 * Aggregator that performs logical AND on added values.
 * Supports Boolean, Long, ArrayList<Boolean>, ArrayList<Long>.
 */
class LogicalAndAggregator<T> implements Aggregator<T, T> {
    ...
    public void add(T input) {
        ...
        if (result == null) {
            ...
            if (input instanceof Boolean || input instanceof Long || input instanceof ArrayList) {
                ...
                throw new UnsupportedAggregationException(
                        "AGG_LOGICAL_AND requires Boolean, Long, ArrayList<Boolean>, or ArrayList<Long>, but got: "
        ...
        if (result instanceof ArrayList && input instanceof ArrayList) {
            ...
            if (existingList.size() != newList.size()) {
                throw new UnsupportedAggregationException(
                        "AGG_LOGICAL_AND requires ArrayLists of equal size, but got sizes: "
