...
/**
 * Simple L(east) R(ecently) U(sed) eviction policy ATTENTION: this class is not thread safe
 */
  ...
  public void setCache(Cache cache) {
    ...
    this.accessTimes = new LinkedHashMap<CacheKey, Long>(initialCapacity, 1f, true) {
      ...
      protected boolean removeEldestEntry(Map.Entry<CacheKey, Long> eldest) {
        ...
        // here the cache check is only for performance gain; we are trying to avoid the sequence
        // add + poll + hasCacheKey
