 ...
 * <p>
 * This condition works with the {@link EnabledOnEnv} and {@link SkipOnEnv} annotations
 * to conditionally execute tests depending on the current test environment
 * (e.g., Docker, Redis Enterprise).
 * <p>
...
public class EnvCondition implements ExecutionCondition {
  ...
  private ConditionEvaluationResult evaluateEnabledOnEnv(EnabledOnEnv annotation, String currentEnv) {
    ...
    if (isEnabled) {
      logger.debug("Test enabled: current environment '{}' matches one of {}",
          currentEnv, Arrays.toString(enabledEnvs));
      return ConditionEvaluationResult.enabled(
          "Current environment '" + currentEnv + "' is in the enabled list");
    }
  ...
  private ConditionEvaluationResult evaluateDisabledOnEnv(SkipOnEnv annotation, String currentEnv) {
    ...
    logger.debug("Test enabled: current environment '{}' is not in disabled list {}",
        currentEnv, Arrays.toString(disabledEnvs));
    return ConditionEvaluationResult.enabled(
        "Current environment '" + currentEnv + "' is not in the disabled list");
  }

  /**
   * Retrieves the {@link EnabledOnEnv} annotation from the test method or class.
  ...
  private EnabledOnEnv getEnabledAnnotation(ExtensionContext context) {
    Optional<EnabledOnEnv> methodAnnotation = AnnotationUtils.findAnnotation(
        context.getTestMethod(), EnabledOnEnv.class);
    if (methodAnnotation.isPresent()) {
      return methodAnnotation.get();
    }

    Optional<EnabledOnEnv> classAnnotation = AnnotationUtils.findAnnotation(
        context.getRequiredTestClass(), EnabledOnEnv.class);
    return classAnnotation.orElse(null);
  }

  /**
   * Retrieves the {@link SkipOnEnv} annotation from the test method or class.
   * Method-level annotations take precedence over class-level annotations.
   *
   * @param context the extension context
   * @return the annotation, or null if not present
  ...
  private SkipOnEnv getDisabledAnnotation(ExtensionContext context) {
    Optional<SkipOnEnv> methodAnnotation = AnnotationUtils.findAnnotation(
        context.getTestMethod(), SkipOnEnv.class);
    if (methodAnnotation.isPresent()) {
      return methodAnnotation.get();
    }

    Optional<SkipOnEnv> classAnnotation = AnnotationUtils.findAnnotation(
        context.getRequiredTestClass(), SkipOnEnv.class);
    return classAnnotation.orElse(null);
  }
}
