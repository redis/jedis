 ...
 * <p>
 * This interface is used internally by the cluster command executor to resolve connections
 * based on the command type (keyed vs keyless) and read preference configuration.
 */
interface ConnectionResolver {
   ...
   * @return ConnectionIntent.READ if the command has the READONLY flag, otherwise ConnectionIntent.WRITE
   */
  default ConnectionIntent getIntent(CommandObject<?> command, CommandFlagsRegistry flags) {
    return flags.getFlags(command.getArguments())
