...
interface RawJedis {
     ...
     * possible to remove the timeout using the {@link #persist(byte[])}
     * command.
     * <p>
     * Time complexity: O(1)
     * 
     ...
     * terminology.
     * <p>
     * {@link #expireAt(byte[], long) expireAt} has the same effect and semantic
     * as {@link #expire(byte[], long) expire}, but instead of specifying the
     * number of seconds representing the TTL (time to live), it takes an
     ...
     *             if key is null
     */
    Boolean expireAt(final byte[] key, final long unixTime);
     ...
     * of the list, or start greater then end, an empty list is returned. If end
     * is over the end of the list Redis will threat it just like the last
     * element of the list.
     * <p>
    ...
    /**
     * Move key from the currently selected database (see {@link #select(long)
     * select}) to the speficied destination database.
     * <p>
     ...
     *             if index is not 0 <= index < 15
     */
    Boolean move(final byte[] key, final long index);
     ...
     */
    Boolean msetnxRaw(Pair<byte[], byte[]> keyValuePair1,
     ...
     * {@link #msetnxRaw(Pair, Pair...) MSETNX} if you don't want to overwrite
     * existing values.
     * <p>
     ...
     *             if keyValuePair1 or its members are null
     */
    Boolean msetRaw(Pair<byte[], byte[]> keyValuePair1,
	    Pair<byte[], byte[]>... keyValuePairN);
     ...
     *             if key is null
     */
    Boolean persist(final byte[] key);
     ...
     */
    Boolean rename(final byte[] srcKey, final byte[] dstKey);
     ...
     *             if srcKey or dstKey is null
     */
    Boolean renamenx(final byte[] srcKey, final byte[] dstKey);
     ...
     */
    Boolean sadd(final byte[] key, final byte[] value);
     ...
     *         set is given it behaves as {@link #smembers(byte[])}.
     * @throws JedisException
     ...
     * <p>
     * This command works exactly like SINTER but instead of being returned the
     ...
     * <p>
     * For default every new client connection is automatically selected to DB
     * 0.
     * 
     * @param index
     *            the DB to select, 0 - 15
     * @return true, command can not fail
     * @throws IllegalArgumentException
     *             if index is not 0 - 15
     */
    Boolean select(final long index);
     ...
     * result as {@link #smembers(byte[])}. Actually smembers is just syntax
     * sugar for sinter.
     * <p>
     ...
     */
    Set<byte[]> sinter(final byte[] key1, final byte[]... keyN);
     ...
     * resulting set is stored as dstkey.
     * <p>
     * Time complexity: O(N*M) worst case where N is the cardinality of the
     * smallest set and M the number of sets
     * 
     ...
     */
    Long sinterstore(final byte[] dstKey, final byte[] key1,
	    final byte[]... keyN);
     ...
     *            command produces the same result as {@link #smembers(byte[])}
     * 
     * @param keyN
     *            all other keys
     * 
     ...
     * <p>
     * This command works exactly like SUNIUN but instead of being returned the
     * resulting set is stored as dstkey. Any existing value in dstkey will be
     * over-written.
     * <p>
     * Time complexity O(N) where N is the total number of elements in all the
     ...
     * that has an {@link #expire(byte[], long) EXPIRE} set. This introspection
     ...
     *             if key or value is null
     */
    Boolean zadd(final byte[] key, final double score, final byte[] value);
     ...
     * {@link #zrangeByScore(byte[], byte[], byte[])}.
     * <p>
     ...
     * treat it like it is the last element of the sorted set.
     * <p>
     * See {@link #zrange(byte[], long, long)} when you need only the members.
     * <p>
     * See {@link #zrevrangeWithScores(byte[], long, long)} when you need the
     ...
     * <p>
     * Use {@link #zrevrange(byte[], long, long)} to get the rank of an element
     ...
     * members and scores.
     * <p>
     * See {@link #zrange(byte[], long, long)} when you need the members ordered
     ...
     * to the lowest score.
     * <p>
     * See {@link #zrevrange(byte[], long, long)} when you need only the
     * members.
     * <p>
     * See {@link #zrangeWithScores(byte[], long, long)} when you need the
     ...
     * <p>
     * Use {@link #zrange(byte[], long, long)} to get the rank of an element
