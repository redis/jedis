 ...
 * throughput number per client configuration:
 * <ul>
 ...
 * <li>{@link RedisClientCSCT1}/{@link RedisClientCSCT8} — pooled RedisClient with client-side
 * cache.</li>
 * </ul>
 * This is a user-facing comparison benchmark. Workload parameters are fixed by design..
 * <p>
 * Workload:
 * <ul>
 ...
 * </ul>
 * <p>
 * Run with: {@code mvn -Pjmh-headline test -Djmh.includes="GetSetMixedR90W10Benchmark"}.
 * <p>
 * WARNING: {@code @Setup} calls {@code FLUSHDB} on the target Redis instance. Do not run against a
...
public abstract class GetSetMixedR90W10Benchmark {
  ...
  /**
   * Standalone Jedis baseline. T1 only — {@link Jedis} is not thread-safe, so this configuration
   * cannot be safely run with @Threads > 1. Pool/concurrency benefits are demonstrated by the
   * RedisClient variants.
   */
  @Threads(1)
  ...
  /**
   * Shared setup for {@link JedisPool} variants. Each benchmark invocation borrows a {@link Jedis}
   * from the pool, runs the operation, and returns it via try-with-resources. Abstract — JMH skips
   * it.
   */
