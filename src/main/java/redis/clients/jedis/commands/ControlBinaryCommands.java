...
public interface ControlBinaryCommands extends AccessControlLogBinaryCommands, ClientBinaryCommands {
  ...
  /**
   * Provide information on the role of a Redis instance in the context of replication, by returning
   * if the instance is currently a master, slave, or sentinel. The command also returns additional
   * information about the state of the replication (if the role is master or slave) or the list of
   * monitored master names (if the role is sentinel).
  ...
  /**
   * Returns the time in seconds since the last access to the value stored at {@code key}. The
   * command is only available when the maxmemory-policy configuration directive is not set to one
   * of the LFU policies.
   * @param key The key in Redis server
   ...
   * <p>
   * The command is only available when the maxmemory-policy configuration directive is set to one
   * of the LFU policies.
   * @param key The key in Redis server
  ...
  /**
   * Reports about different memory-related issues that the Redis server experiences, and advises
   * about possible remedies.
   */
  byte[] memoryDoctorBinary();

  /**
   * Reports the number of bytes that a key and its value require to be stored in RAM. The reported
   * usage is the total of memory allocations for data and administrative overheads that a key its
   * value require.
   * <p>
   ...
   * value require.
   * <p>
   * See for details: <a href="https://redis.io/commands/memory-usage">MEMORY USAGE key SAMPLES
   * count</a>
   * @param key The key in Redis server
   * @return The memory usage in bytes, or nil when the key does not exist
   */
