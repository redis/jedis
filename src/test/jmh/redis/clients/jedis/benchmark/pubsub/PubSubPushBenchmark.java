 ...
 * <p>
 * Measures end-to-end throughput of the pub/sub pipeline:
 * 
 * <pre>
 *   publisher.publish()
 *       → server fan-out
 ...
 * <li>Integration benchmark regresses (alone) → network or subscriber thread dispatch logic
 * regressed.</li>
 * <li>Both regress together → both paths share the affected code.</li>
 * </ul>
 * <p>
 * <b>Why single-threaded.</b> Pub/sub throughput on one channel is bounded by the single-threaded
 * subscriber connection's read+dispatch loop — that loop is exactly what we want to track
 * <p>
 * Throughput here measures pub/sub round-trips per second (publish → receive).
 * <p>
 * WARNING: {@code @Setup} flushes the target Redis. Do not run against shared Redis.
 */
...
public class PubSubPushBenchmark {
  ...
  /**
   * Single-slot hand-off between the {@link JedisPubSub} callback (on the subscriber thread) and
   * the benchmark thread.
   * <p>
   * {@link SynchronousQueue} guarantees strict 1:1 message-to-take pairing: each {@code onMessage}
   * {@code put()} blocks until the benchmark calls {@code take()}, preventing the subscriber from
   * running ahead and ensuring every reported op represents exactly one round-trip.
   */
  ...
  /**
   * Publishes one message and waits for the subscriber callback to fire. Throughput score = pub/sub
