...
/**
 * Aggregator that returns the maximum value added so far.
...
class MaxAggregator<T> implements Aggregator<T, T> {
    ...
    public void add(T input) {
        ...
        throw new UnsupportedAggregationException(
                "AGG_MAX policy requires Comparable types or KeyValue, but got: "
                        + max.getClass().getName() + " and " + input.getClass().getName());
    }
    ...
    private KeyValue<?, ?> aggregateKeyValueMax(KeyValue<?, ?> kv1, KeyValue<?, ?> kv2) {
        Object maxKey = ((Comparable<Object>) kv1.getKey()).compareTo(kv2.getKey()) >= 0 ? kv1.getKey() : kv2.getKey();
        Object maxValue = ((Comparable<Object>) kv1.getValue()).compareTo(kv2.getValue()) >= 0 ? kv1.getValue() : kv2.getValue();
