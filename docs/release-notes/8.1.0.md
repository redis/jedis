...
`cmsInitByDim` and `cmsInitByProb` gain overloads with a trailing `int cellSize` (1, 2, 4 or 8),
mapping to the new `CELL_SIZE` option of `CMS.INITBYDIM` / `CMS.INITBYPROB` (Redis 8.12). Any
other value is rejected client-side with `IllegalArgumentException`. Smaller cells cut memory for
short-lived counters; 8-byte cells allow very large totals. `cmsInfo` now
