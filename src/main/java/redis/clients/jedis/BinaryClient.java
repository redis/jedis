...
public class BinaryClient extends Connection {
  ...
  public void set(final byte[] key, final byte[] value, final byte[] nxxx, final byte[] expx,
    ...
    public void sinterstore(final byte[] dstkey, final byte[]... keys) {
        sendInterUnionStore(SINTERSTORE, dstkey, keys);
    }
    ...
    public void sunionstore(final byte[] dstkey, final byte[]... keys) {
        sendInterUnionStore(SUNIONSTORE, dstkey, keys);
    }
    ...
    public void zcount(final byte[] key, final double min, final double max) {
        sendCommandRangeAndCountSimple(ZCOUNT, key, min, max);
    }
    ...
    public void zrangeByScore(final byte[] key, final double min, final double max) {
        sendCommandRangeAndCountSimple(ZRANGEBYSCORE, key, min, max);
    }
    ...
    public void zrevrangeByScore(final byte[] key, final double max, final double min) {
        sendCommandRangeAndCountSimple(ZREVRANGEBYSCORE, key, max, min);
    }
    ...
    public void zrangeByScore(final byte[] key, final double min, final double max, final int offset,
                              int count) {
        sendCommandZrangeByScoreAndZrevRangeByScoreGenerics(ZRANGEBYSCORE, key, min, max, offset, count);
    }

    public void zrevrangeByScore(final byte[] key, final double max, final double min,
                                 final int offset, int count) {
        sendCommandZrangeByScoreAndZrevRangeByScoreGenerics(ZREVRANGEBYSCORE, key, max, min, offset, count);
    }
    ...
    public void zrangeByScoreWithScores(final byte[] key, final double min, final double max) {
        sendCommandZrangeAndRevZrangeScore(ZRANGEBYSCORE, key, min, max);
    }
    ...
    public void zrevrangeByScoreWithScores(final byte[] key, final double max, final double min) {
        sendCommandZrangeAndRevZrangeScore(ZREVRANGEBYSCORE, key, max, min);
    }
    ...
    public void zunionstore(final byte[] dstkey, final ZParams params, final byte[]... sets) {
        sendCommandZunionAndZinter(ZUNIONSTORE, dstkey, params, sets);
    }

    public void zinterstore(final byte[] dstkey, final ZParams params, final byte[]... sets) {
        sendCommandZunionAndZinter(ZINTERSTORE, dstkey, params, sets);
    }
  ...
  public Long getDB() {
    ...
    private void sendCommandZrangeByScoreAndZrevRangeByScoreGenerics(Command command, final byte[] key, final double min, final double max,
                                                                     final int offset, final int count) {
        ...
        sendCommand(command, key, byteArrayMin, byteArrayMax, LIMIT.raw, toByteArray(offset),
                toByteArray(count), WITHSCORES.raw);
    }

    private void sendCommandZunionAndZinter(Command command, final byte[] dstkey, final ZParams params, final byte[]... sets) {
        final List<byte[]> args = new ArrayList<byte[]>();
        args.add(dstkey);
    ...
    private void sendInterUnionStore(Command command, byte[] dstkey, byte[][] keys) {
    ...
    private void sendCommandZrangeAndRevZrangeScore(Command command, final byte[] key, final double max, final double min) {
        byte[] byteArrayMin = (min == Double.NEGATIVE_INFINITY) ? "-inf".getBytes() : toByteArray(min);
        byte[] byteArrayMax = (max == Double.POSITIVE_INFINITY) ? "+inf".getBytes() : toByteArray(max);
        sendCommand(command, key, byteArrayMax, byteArrayMin, WITHSCORES.raw);
    }

    private void sendCommandScan(Command command, final byte[] key, final byte[] cursor, final ScanParams params) {
        final List<byte[]> args = new ArrayList<byte[]>();
        args.add(key);
        args.add(cursor);
        args.addAll(params.getParams());
        sendCommand(command, args.toArray(new byte[args.size()][]));
    }

    private void sendCommandRangeAndCountSimple(Command command, final byte[] key, final double min, final double max) {
        byte[] byteArrayMin = (min == Double.NEGATIVE_INFINITY) ? "-inf".getBytes() : toByteArray(min);
        byte[] byteArrayMax = (max == Double.POSITIVE_INFINITY) ? "+inf".getBytes() : toByteArray(max);
        sendCommand(command, key, byteArrayMin, byteArrayMax);
    }
}
