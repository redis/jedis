 ...
 * which ones {@link HashImport#close()} has queued to be discarded on the connection's next borrow.
 * Pure state &mdash; it issues no commands; {@link Connection} performs the actual {@code DISCARD}
 * I/O.
 * <p>
...
class HimportConnectionState {
  ...
  void enqueueDiscard(String fieldset) {
   ...
   * them from the prepared set), for the caller to {@code DISCARD}. Empty and allocation-free in
   * the common case where nothing was queued.
   */
