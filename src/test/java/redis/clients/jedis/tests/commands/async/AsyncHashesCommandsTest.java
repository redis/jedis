...
public class AsyncHashesCommandsTest extends AsyncJedisCommandTestBase {
	...
	asyncJedis.hmget(STRING_LIST_CALLBACK.withReset(), "foo", "bar", "car",
		"foo");
	...
	asyncJedis.hmget(BYTE_ARRAY_LIST_CALLBACK.withReset(), bfoo, bbar,
		bcar, bfoo);
	...
	List<byte[]> bvals = BYTE_ARRAY_LIST_CALLBACK
		.getResponseWithWaiting(1000);
	...
	Map<String, String> hash = STRING_MAP_CALLBACK
		.getResponseWithWaiting(1000);
	...
	Map<byte[], byte[]> bhash = BYTE_ARRAY_MAP_CALLBACK
		.getResponseWithWaiting(1000);
