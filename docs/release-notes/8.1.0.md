...
### Infinity arguments use the protocol encoding

Positive and negative infinity arguments now use `SafeEncoder` instead of the JVM's default
charset. With the default `SafeEncoder` charset (UTF-8), `+inf` and `-inf` are encoded as four
bytes even when the JVM default is UTF-16. Previously, that JVM default produced invalid
infinity arguments for sorted-set commands.
