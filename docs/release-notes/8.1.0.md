...
### URI credential components are split on the raw userinfo and percent-decoded ([#4736](https://github.com/redis/jedis/pull/4736))

`JedisURIHelper` previously split the user/password boundary on the *decoded*
userinfo, so a percent-encoded colon inside the username shifted the boundary:
`redis://us%3Aer:pw@host` authenticated as user `us` with password `er:pw`
instead of user `us:er` with password `pw`. The boundary is now located in the
raw (still percent-encoded) userinfo and each component is percent-decoded
afterwards, without `URLDecoder`'s form-style `+` handling. Literal non-ASCII
characters mixed with percent escapes are preserved.

**Action required:** none unless an application depended on the old boundary
shift for usernames containing a percent-encoded colon; such credentials now
authenticate with the intended user/password split.
