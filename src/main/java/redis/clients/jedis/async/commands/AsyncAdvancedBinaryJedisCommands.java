...
public interface AsyncAdvancedBinaryJedisCommands {
    ...
    void objectRefcount(final AsyncResponseCallback<Long> callback,
	    final byte[] key);

    void objectEncoding(final AsyncResponseCallback<byte[]> callback,
	    final byte[] key);

    void objectIdletime(final AsyncResponseCallback<Long> callback,
	    final byte[] key);

    // server section

    void configGet(final AsyncResponseCallback<List<byte[]>> callback,
	    final byte[] pattern);

    void configSet(final AsyncResponseCallback<String> callback,
	    final byte[] parameter, final byte[] value);
