...
flowchart TB
    ...
    subgraph ClientLayer["Client Layer"]
        ...
        RSC["RedisSentinelClient<br/>(Sentinel)"]
...
- **Use Case:** the deprecated `UnifiedJedis(Connection)` constructor (single fixed
  connection). Note: `Pipeline` and `Transaction` do not use a `CommandExecutor` at
  all — they queue commands directly on their `Connection`.
...
**Characteristics:**
- Retry logic with deadline-proportional backoff (the remaining time budget is
  spread across the remaining attempts)
- Configurable `maxAttempts` and `maxTotalRetriesDuration`
- Handles `JedisConnectionException` with backoff (sleeps only after 2+
  consecutive connection failures)
- Deadline-based retry termination
- **Use Case:** wired via the `UnifiedJedis(provider, maxAttempts,
  maxTotalRetriesDuration)` constructor. Not currently created by
...
- **Use Case:** supplying an externally managed `Connection` to `UnifiedJedis`
  (advanced/custom wiring). Note: `Pipeline` and `Transaction` receive their
  `Connection` directly, without a provider.
...
- Each database has:
  ...
  - `weight` - selection priority (higher weight preferred)
...
For standalone retries, construct `UnifiedJedis(provider, maxAttempts,
maxTotalRetriesDuration)` directly, which wires a `RetryableCommandExecutor`.
...
- Master discovery through sentinels
- Automatic reconnection to the new master on `+switch-master`
- Separate `sentinelClientConfig(...)` for connecting to the sentinel nodes
...
- `cacheConfig` / `cache` - Client-side caching configuration
- `connectionProvider` / `commandExecutor` - custom component overrides

(Retry settings `maxAttempts` / `maxTotalRetriesDuration` are declared on
...
### Example 2: GET Command with Retry (UnifiedJedis with RetryableCommandExecutor)
```
client.get("mykey")
  ↓
UnifiedJedis.get("mykey")
