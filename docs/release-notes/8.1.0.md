...
### Comparing GEO responses without coordinates

`GeoRadiusResponse.equals()` now compares responses that omit `WITHCOORD`
without throwing `NullPointerException`. Two missing coordinates compare equal;
a missing coordinate and a present coordinate compare unequal.
