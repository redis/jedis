...
public interface ArrayPipelineCommands {
   ...
   * in one or more inclusive ranges of indices.
   * <p>
   ...
   * within an inclusive index range using one or more textual predicates.
   * <p>
   ...
   * @return a {@link Response} that resolves to the matching indices in traversal order, or
   *         alternating index/value pairs when {@code WITHVALUES} is set
   * @since 8.0
   */
   ...
   * Time complexity: O(1)
   * @param key the name of the key that holds the array
   * @return a {@link Response} that resolves to the next insert index, {@code 0} for missing keys,
   *         or {@code null} when the cursor is exhausted
   * @since 8.0
   */
   ...
   * over the non-empty elements in {@code [start, end]}. Use this overload for operations that take
   * no extra argument.
   * <p>
   * Time complexity: O(N) where N is the number of elements scanned.
   * @param key the name of the key that holds the array
   ...
   * @return a {@link Response} that resolves to the operation's result; type depends on {@code op}
   *         (see {@link ArrayCommands#arop(String, long, long, ArrayOp)})
   * @since 8.0
   */
   ...
   * {@code AROP key start end MATCH value}).
   * <p>
   * Time complexity: O(N) where N is the number of elements scanned.
   * @param key the name of the key that holds the array
   * @param start zero-based start index of the range
   ...
   * in an inclusive index range and returns a flat array of alternating index/value pairs.
   * <p>
  ...
  /**
   * <b><a href="https://redis.io/commands/arscan">ARSCAN Command</a></b> Iterates existing elements
   * in an inclusive index range with a cap on the number of elements returned.
   * <p>
