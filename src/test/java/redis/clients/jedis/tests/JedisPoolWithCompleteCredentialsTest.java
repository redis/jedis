...
public class JedisPoolWithCompleteCredentialsTest {
  ...
  public void checkConnections() {
    JedisPool pool =
        new JedisPool(new JedisPoolConfig(), hnp.getHost(), hnp.getPort(), "acljedis", "fizzbuzz");
  ...
  public void checkResourceWithConfigIsClosableAndReusable() {
    ...
    try (JedisPool pool = new JedisPool(config, hnp, DefaultJedisClientConfig.builder()
        .user("acljedis").password("fizzbuzz").clientName("closable-resuable-pool").build())) {
  ...
  public void securePool() {
    ...
    JedisPool pool =
        new JedisPool(config, hnp.getHost(), hnp.getPort(), 2000, "acljedis", "fizzbuzz");
    try (Jedis jedis = pool.getResource()) {
      jedis.set("foo", "bar");
    }
    pool.close();
  ...
  public void securePoolNonSSL() {
    ...
    config.setTestOnBorrow(true);
    JedisPool pool =
        new JedisPool(config, hnp.getHost(), hnp.getPort(), 2000, "acljedis", "fizzbuzz", false);
    try (Jedis jedis = pool.getResource()) {
      jedis.set("foo", "bar");
    }
    pool.close();
  ...
  public void testResetInvalidPassword() {
    JedisFactory factory = new JedisFactory(hnp.getHost(), hnp.getPort(), 2000, 2000, "acljedis",
        "fizzbuzz", 0, "my_shiny_client_name") {
    };
