...
public class BinaryValuesCommandsTest extends JedisCommandTestBase {
	...
	public void setUp() throws Exception {
		super.setUp();
		final InputStream is = this.getClass().getResourceAsStream("/MySimpson.png");
		byte[] resource = new byte[512];//29542
		byte[] buffer = new byte[2048];
		int length = 0;
		int idx = 0;
		while(-1 != (length = is.read(buffer))) {
			if ((idx + length) > resource.length) {
				byte[] newresource = new byte[resource.length + ((idx + length) - resource.length)];
				System.arraycopy(resource, 0, newresource, 0, idx);
				resource = newresource;
			}
			System.arraycopy(buffer, 0, resource, idx, length);
			idx += length;
		}
		binaryValue = resource;
	}
    ...
    public void mget() {
		...
		JedisTest.isListAreEquals(expected, values);
