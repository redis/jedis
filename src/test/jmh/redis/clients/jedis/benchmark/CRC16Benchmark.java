 ...
 *
 * Tests the performance of Redis Cluster hash slot computation using a realistic mix of key patterns.
 * Each benchmark invocation rotates through different key types (short, medium, long, hashtag, etc.)
 * to simulate real-world workload distribution.
 *
 * Compares String-based vs byte[]-based slot calculation.
 *
...
public class CRC16Benchmark {
     ...
     * - Short numeric (common)
     * - Medium alphanumeric (typical)
     * - Long alphanumeric (worst case for CRC16)
     * - Repeated characters (pathological case)
     * - Typical string value
     * - Hash tag keys (cluster routing)
     */
    private static final String[] STRING_TEST_SET = {
            "",                                          // Empty (edge case)
    ...
    /**
     * Pre-encoded byte arrays for testing byte[]-based slot calculation.
     * Avoids UTF-8 encoding overhead during benchmark execution.
     */
    ...
    /**
     * Benchmark String-based slot calculation with mixed key patterns.
     * Performs BATCH_SIZE operations per invocation to reduce measurement overhead.
     * Rotates through test set to simulate realistic workload.
     */
    @Benchmark
    ...
    /**
     * Benchmark byte[]-based slot calculation with mixed key patterns.
     * Performs BATCH_SIZE operations per invocation to reduce measurement overhead.
     * Tests performance when key is already available as byte array.
     * This is more efficient as it avoids UTF-8 encoding overhead.
     */
    @Benchmark
    @OperationsPerInvocation(BATCH_SIZE)
