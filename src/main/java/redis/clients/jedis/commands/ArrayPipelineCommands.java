...
public interface ArrayPipelineCommands {
   ...
   * Deletes elements in one or more inclusive ranges of indices.
   * <p>
   ...
   * @since 8.0
   */
  Response<Long> ardelrange(String key, long[]... ranges);
   ...
   * @return a {@link Response} that resolves to the value at the given index, or
   *         {@code null} if the key or index does not exist
   * @since 8.0
   */
   ...
   * Returns the values at a contiguous range of indices.
   * <p>
   ...
   * @return a {@link Response} that resolves to the list of values in traversal
   *         order; empty slots appear as {@code null} entries
   * @since 8.0
   */
   ...
   * @return a {@link Response} that resolves to the matching indices in traversal
   *         order, or alternating index/value pairs when {@code WITHVALUES} is set
   * @since 8.0
   */
  Response<List<Object>> argrep(String key, long start, long end, ArgrepParams params);

  /**
   * <b><a href="https://redis.io/commands/arinfo">ARINFO Command</a></b>
   * Returns metadata describing an array.
   * <p>
   * Time complexity: O(1)
   * @param key the name of the key that holds the array
   * @return a {@link Response} that resolves to a map of metadata fields to their values
   * @since 8.0
   */
  Response<Map<String, Object>> arinfo(String key);
   ...
   * <p>
   * Time complexity: O(1) without {@code FULL}; O(S) with {@code FULL}
   ...
   * @since 8.0
   */
  Response<Map<String, Object>> arinfo(String key, boolean full);
   ...
   * current insert cursor position.
   * <p>
   ...
   * Time complexity: O(1)
   * @param key the name of the key that holds the array
   * @return a {@link Response} that resolves to the array length, or {@code 0} if the key
   *         does not exist
   * @since 8.0
   */
   ...
   * Returns the values at multiple indices.
   * <p>
   * Time complexity: O(N) where N is the number of requested indices.
   * @param key the name of the key that holds the array
   * @param indices one or more zero-based indices to retrieve
   * @return a {@link Response} that resolves to the list of values aligned with
   *         {@code indices}; missing slots appear as {@code null}
   * @since 8.0
   */
   ...
   * Sets multiple index-value pairs.
   * <p>
   ...
   * Time complexity: O(1)
   * @param key the name of the key that holds the array
   * @return a {@link Response} that resolves to the next insert index, {@code 0} for missing
   *         keys, or {@code null} when the cursor is exhausted
   * @since 8.0
   */
   ...
   * {@code [start, end]}. Use this overload for operations that take no
   * extra argument.
   * <p>
   * Time complexity: O(N) where N is the number of elements scanned.
   * @param key the name of the key that holds the array
   ...
   * @return a {@link Response} that resolves to the operation's result; type depends on
   *         {@code op} (see {@link ArrayCommands#arop(String, long, long, ArrayOp)})
   * @since 8.0
   */
  Response<Object> arop(String key, long start, long end, ArrayOp op);

  /**
   * <b><a href="https://redis.io/commands/arop">AROP Command</a></b>
   * Counts elements equal to {@code value} within {@code [start, end]}
   * (equivalent to {@code AROP key start end MATCH value}).
   * <p>
   * Time complexity: O(N) where N is the number of elements scanned.
   * @param key the name of the key that holds the array
   * @param start zero-based start index of the range
   ...
   * @since 8.0
   */
  Response<Long> aropMatch(String key, long start, long end, String value);
   ...
   * flat array of alternating index/value pairs.
   * <p>
   ...
   * @param end zero-based end index (inclusive)
   * @return a {@link Response} that resolves to {@code [idx1, val1, idx2, val2, ...]}
   * @since 8.0
   */
  Response<List<Object>> arscan(String key, long start, long end);
   ...
   * @since 8.0
   */
  Response<List<Object>> arscan(String key, long start, long end, long limit);
   ...
   * @return a {@link Response} that resolves to {@code 1} if the cursor was set,
   ...
   * Sets one or more contiguous values starting at {@code index}.
   * <p>
