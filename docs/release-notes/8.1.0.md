...
`CommandArguments` now accepts an optional expected argument count:

```java
CommandArguments args = new CommandArguments(Command.SET, 8); // pre-size for up to 8 arguments
```

or the builder-style `expectArguments(int)` on an existing instance. The count is a hint
used to pre-size the backing argument list, avoiding repeated `ArrayList` growth copies for
commands built from large collections (e.g. `MSET`, map `HSET`/`HMSET`, `XADD`). Under-estimating
falls back to normal growth, so correctness never depends on the estimate. Custom commands and
overrides of `CommandObjects.commandArguments(ProtocolCommand)` are unaffected.
