...
public abstract class BinaryShardedJedisBase implements BinaryJedisCommands {

    private Sharding<Jedis, JedisShardInfo> sharded;

    public BinaryShardedJedisBase(Sharding<Jedis, JedisShardInfo> sharded) {
    this.sharded = sharded;
    }

    public void disconnect() {
	for (Jedis jedis : sharded.getAllShards()) {
	    jedis.quit();
	    jedis.disconnect();
	}
    }
    ...
    return sharded.getAllShards();
    }

    public String set(byte[] key, byte[] value) {
	Jedis j = sharded.getShard(key);
