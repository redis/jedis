 ...
 * <p>
 * This test class follows the pattern of {@link SSLACLJedisIT} but adapted for {@link RedisClient}.
 * <p>
 * This test is only executed when the server/cluster is Redis 6.0.0 or more.
 */
...
public class ACLRedisClientIT {
  ...
  private static final String trustStoreName = ACLRedisClientIT.class.getSimpleName();
  ...
  public void connectWithSsl() {
    try (RedisClient client = RedisClient.builder()
        ...
        .clientConfig(DefaultJedisClientConfig.builder().ssl(true).user(endpoint.getUsername())
  ...
  public void connectWithUrl() {
    ...
    try (RedisClient client = RedisClient
        .create(endpointWithDefaultUser.getURIBuilder().defaultCredentials().build().toString())) {
      assertEquals("PONG", client.ping());
    }
  ...
  public void connectWithUri() {
    ...
    // Test with default user endpoint
    try (RedisClient client = RedisClient
        .create(endpointWithDefaultUser.getURIBuilder().defaultCredentials().build())) {
      assertEquals("PONG", client.ping());
    }
    // Test with ACL user endpoint
    try (RedisClient client = RedisClient
